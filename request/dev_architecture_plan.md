...
이번 문서는 `출제 후보 04. 설비 정비주기 예측형`을 기준으로 작성하지만, 최종 대회에서는 4개 문제 또는 다회차 운영 가능성이 있다.
...
참가자 1만 명, 공식 제출 1일 3회, private 데이터 30행을 행별 호출하면 단순 상한으로 하루 최대 90만 회 호출이 발생할 수 있다.
...
최종 형태는 미송님 및 주최측이 아래 기준으로 결정한다.
...
```text
참가자 10,000명 × 공식 제출 3회/일 × private 데이터 30행
= 최대 900,000회 호출/일
```
...
| 5~10행 미니배치 | 비용과 안정성 균형 | 파싱 로직 필요 | 운영 우선 검토 |
| 30행 일괄 호출 | 호출 수 최소화 | 출력 실패 시 리스크 큼 | 실험 후 적용 |
