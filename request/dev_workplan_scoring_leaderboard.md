...
- ⚠️ **`max_tokens=50`이 프롬프트 품질 사다리를 깨뜨린다(실측).** 도메인 추론을 유도하는 822자 지침이 211자 지침보다 낮게 나온다(0.1738 < 0.2337) — INVALID 151행이 전부 69~91자에서 절단됐다. 판정 오류가 아니라 출력 절단이므로 **`max_tokens` 상향 또는 고정 시스템 지시의 출력 형식 강제**를 검토해야 한다(→ `reference_prompts.md`).
