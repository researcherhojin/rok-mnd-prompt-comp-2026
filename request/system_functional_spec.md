...
>
> **중요:** 본 문서는 개발 확정 명세가 아니라, 자문단 기준의 기능 설계안이다. 최종 기능 범위는 주최측 확인 후 확정한다. 특히 배점, 모델, 하네스 형태(Shape A/B), private 데이터 행 수, 리더보드 공개 정책은 아직 확정 전제로 두지 않는다.
...
```json
{
  "instruction": "너는 K-511 계열 군용 트럭의 부품 계통별 정비 분석가다. 입력 데이터를 보고 risk_grade와 cycle_range를 판단하라. 입력: {{input}} 출력: HIGH, 0-30 형식으로만 답하라.",
  "memo": "선택: 접근 방식 요약"
}
```
...
- LLM 출력
- 통과/실패
- 완전일치 행 수 · 평균 Macro F1 (정확도와 F1을 분리 표기 · 참고용)
...
- “Macro F1이 무엇인가요?”
- “maintenance_count_1y와 operation_hours_total은 어떤 의미인가요?”
...
> **주의:** 아래 산식은 참고 구조다. 최종 배점은 주최측 확정 후 반영한다.
...
| `scoring_jobs` | submission_id, status, attempts, error_message, started_at, finished_at | 큐 작업 · `started_at`+타임아웃으로 좀비(orphaned `running`) 재조정 |
...
| private 행 수 | Public 300 / Private 700 설명용 | 운영용 확장 여부 |
| 미리보기 | 50회/일 가정 | 최종 제한 |
| 제출 | 3회/일 가정 | 최종 제한 |
