...
1. **채점 모델** — 경량 모델 우선 검토(예: `gpt-4o-mini`, 대안 `gpt-4.1-nano`). 실제 운영 모델은 비용·속도·응답 안정성 테스트 후 확정. ⚠️ **모델별 점수 천장 실측**: 동일 규칙 프롬프트가 소형(nano) ≈35 / mini ≈54 / 상위(gpt-5.4급) ≈80 — 소형 고정 시 완벽한 규칙도 상한이 낮아 변별력이 떨어진다. 모델 선택이 점수 분포를 좌우(→ `reference_prompts.md`). 상세·비용 → `dev_workplan_scoring_leaderboard.md`
