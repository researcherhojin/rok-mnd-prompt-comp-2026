# K-511 계통별 정비주기 예측형 — 대회 설계 기획서

## Context (왜 이 문서를 만드는가)

「2026 국방AI 프롬프트 경진대회」(주최 국방부, 주관 IITP/KAIST·데이원컴퍼니)의
**K-511 계통별 정비주기 예측형**(K-511 계열 군용 트럭 부품 계통별)을 실제 운영 가능한 수준으로 구체화한다.

원 기획안·개발문서(→ `docs/`)에서 미확정으로 남아 있는 항목을 본 문서에서 **설계안**으로 정리한다. 단, 주최측 확인이 필요한 항목은 별도 표기한다.

1. **라벨 체계·샘플 데이터 미정** — `risk_grade`, `cycle_range`의 라벨 값과 예시 규칙을 §1에서 설계한다.
2. **자동채점 기준** — 주최측 공식 자동채점 기준(예측 성능 40·결과 신뢰성 25·데이터 활용역량 20·문제해결 완성도 10·제출규격 5 = 100)을 **참고 기준**으로 반영하되, 7/5 자문단 논의에 따라 **리더보드 점수 / 토탈 점수 분리**와 **프롬프트 효율성 반영 여부**는 주최측 확정이 필요하다.
...
| 파일 | 라벨 | 공개 | 용도 | 행 수 |
|---|---|---|---|---|
| `sample.csv` | O | **공개** | 참가자가 라벨 규칙을 추론하는 **샘플 데이터** | 10 |
| `public.csv` | 입력만 | **서버 보관·비공개** | 대회 중 하네스가 참가자 지침을 재실행하는 **비공개 데이터**(Public 채점) | 300 |
| `private.csv` | 입력만 | **서버 보관·비공개** | 마감 후 재채점하는 **비공개 데이터**(Private 채점) | 700 |
| `ground_truth`(내부) | O | **격리** | 채점 워커만 접근하는 정답셋(Public/Private 분할) | Public 300 / Private 700 |
| `sample_prompt.json` | — | 공개 | 행동 지침 제출 형식 예시 (§3.1) | — |
| `problem_description.md` | — | 공개 | 문제·입력 스키마·라벨 정의·출력 계약·FAQ·보안 유의 | — |

> **파일 위치:** `data/sample.csv` · `data/public.csv` · `data/private.csv` (전체 참조용 `data/all.csv`, 1,000행). seed 20260717.

> **재실행 패러다임:** 참가자는 비공개 데이터를 보지 않는다. 샘플 데이터 10행으로 규칙을 추론해 **일반화된 지침**을 쓰고, 하네스가 숨겨진 비공개 데이터에 적용한다. 비공개 데이터의 입력·정답 모두 비공개 → 하드코딩·역추적 불가.

> **채점셋 규모.** 비공개 채점셋은 **Public 300 / Private 700**(합계 1,000행)이다. LLM 호출 비용을 고려해 공식 채점은 행별 호출이 아니라 미니배치 또는 일괄 배치 호출로 구현하는 것을 검토한다(§7.1·§8.3).

> **실 데이터(`data/`):** `sample.csv`(10·라벨 공개) · `public.csv`(300) · `private.csv`(700) · `all.csv`(1,000·전체 참조용). 라벨 인라인, seed 20260717. 비공개 정답 키(public·private·all)는 git 미추적(로컬 보관), 참가자 공개 `sample.csv`만 추적.

### 1.2 입력 컬럼 (`sample.csv`/`public.csv`/`private.csv` 공통, 라벨 제외)

K-511 계열 군용 트럭의 부품 계통별 정비 이력을 모사한 **합성 데이터**(컬럼 분포·용어는 실제 공공데이터 스타일 감수).
...
전체 스키마는 `id` + 특성 22개 + 라벨 2개 = 25컬럼이며, 채점 하네스에 주어지는 **비-라벨 입력은 23컬럼**(`id` + 특성 22)이다.

| 컬럼 | 타입 | 설명 | 예시 |
|---|---|---|---|
| `id` | int | 제출 대상 ID | 1 |
| `vehicle_id` | str | 차량 식별자 | V-00317 |
| `vehicle_model` | str | 차량 모델 | K-511 또는 K-511A1 |
| `part_system` | str | 부품 계통(8종) | 엔진 / 냉각 / 제동 / 전기 / 변속및구동 / 조향및현가 / 타이어및휠 / 차체및적재 |
| `operation_area` | str | 운용 지역 | 전방 / 후방 / 산악 / 해안 / 도심 |
| `mission_type` | str | 임무 유형 | 수송 / 훈련 / 작전지원 / 보급 |
| `load_condition` | str | 적재 조건 | 경량 / 보통 / 고하중 |
| `observation_month` | int | 관측 월(1~12) | 3 |
| `repair_start_date` | date | 수리 시작일 | 2026-01-05 |
| `repair_end_date` | date | 수리 종료일 | 2026-01-11 |
| `maintenance_cost_krw` | int | 정비 비용(원) | 850000 |
| `last_maintenance_echelon` | int | 최근 정비 제대(1 사용자정비·2 부대정비병정비·3 직접지원정비·4 일반지원정비·5 창정비) | 2 |
| `emergency_repair_flag` | int | 긴급 정비 여부(0 계획·1 긴급) | 0 |
| `maintenance_count_1y` | int | 최근 1년 정비 횟수 | 6 |
| `days_since_last_maintenance` | int | 직전 정비 후 경과일 | 45 |
| `mileage_total_km` | int | 누적 주행거리(km) | 82000 |
| `operation_hours_total` | int | 누적 가동시간 | 4200 |
| `vehicle_age_months` | int | 차량 사용 개월 수 | 96 |
| `part_age_months` | int | 부품 사용 개월 수 | 18 |
| `mileage_since_part_service_km` | int | 부품 정비 후 주행거리(km) | 6400 |
| `hours_since_part_service` | int | 부품 정비 후 가동시간 | 780 |
| `days_since_last_fault` | int | 직전 고장 후 경과일 | 20 |
| `spare_part_lead_time_days` | int | 예비 부품 조달 소요일 | 14 |

### 1.3 라벨 정의 (라벨 값 확정안 · 규칙은 설명/검증용)

두 라벨은 상호 연관(위험↑ → 주기↓).

**① `risk_grade` — 고장위험등급 (3단계)**

> 아래 규칙은 운영자 자동채점의 base_risk 로직이다. 실제 채점셋의 정답 생성 규칙(부품 계통별 임계·함정 보정)은 참가자에게 공개하지 않는다.

base_risk 판정:

| 값 | 판정 규칙(정답 생성) |
|---|---|
| `HIGH` | `maintenance_count_1y ≥ 부품계통별 count_high` **또는** `operation_hours_total ≥ 4,700` **또는** `days_since_last_fault ≤ 25` |
| `LOW` | `maintenance_count_1y ≤ 2` **그리고** `operation_hours_total < 2,700` **그리고** `days_since_last_fault > 90` **그리고** (`mileage_since_part_service_km < 부품계통별 mileage_high` **또는** `hours_since_part_service < 부품계통별 hours_high`) |
| `MEDIUM` | HIGH·LOW 어느 쪽도 아님 |

**② `cycle_range` — 다음 정비주기 구간 (4구간, 일)**

| 값 | 규칙 |
|---|---|
| `0-30` | risk=HIGH |
| `31-90` | risk=MEDIUM **그리고** `operation_hours_total ≥ 2,700` |
| `91-180` | risk=MEDIUM **그리고** `operation_hours_total < 2,700` |
| `181+` | risk=LOW |

> **부품 계통별 임계값**(`mileage_high`/`hours_high`/`count_high`)과 **함정 보정 조건**(비용·조달·계절·긴급 등 11종)의 상세 수치는 **운영자 전용 자동채점 기준**에서 관리하며 **참가자에게 공개하지 않는다**. 본 문서는 계통별 임계 테이블 전체를 나열하지 않고 운영자 관리 대상으로만 참조한다.

> 참가자에게 제공되는 문제 설명서에는 **라벨 값과 출력 형식만 공개**하고, 실제 채점셋의 **정답 생성 규칙은 참가자에게 공개하지 않는다**. 참가자는 샘플 데이터 10행에서 규칙을 **추론**해 지침에 녹인다 = 프롬프트 엔지니어링 역량 평가. 경계 케이스를 배치해 변별.

> ⚠️ **엄밀 플래그 — 라벨 상호결정성(주최측 검토):** `cycle_range`가 `risk_grade`를 결정한다(실측 1,000/1,000: `0-30`→HIGH, `31-90`·`91-180`→MEDIUM, `181+`→LOW). 두 라벨 평균 Macro F1은 HIGH/LOW 경계를 이중 계산하므로 배점 재논의가 필요하다(주최측).

> ⚠️ **데이터 결함 · 재설계 반영(7/5 미팅):** 신규 K-511 데이터는 base_risk 로직 위에 **부품 계통별 임계값**과 **함정 보정 11종**(비용·조달·계절·긴급 등)을 더해 구 단순 규칙 대비 **변별력**을 개선했다. 참가자가 컬럼 설명과 문제 조건을 그대로 LLM에 입력해도 계통별 임계·함정 조건은 추론하기 어렵다. (열린 결정 ⑤ · §7)

---

## 2. 문제 정의 — 고정 하네스, 참가자 슬롯

**핵심.** 자유 프롬프트 제출이 아니다. 본 대회에서 참가자는 코드나 정답 파일을 제출하지 않고, LLM이 데이터를 어떻게 읽고 판단해야 하는지 설명하는 **행동 지침 프롬프트**를 제출한다. 주최 측은 모든 참가자에게 동일하게 적용되는 **고정 시스템 지시**를 구성하고, 그 안에 참가자 행동 지침과 비공개 데이터 입력을 결합해 LLM을 실행한 뒤 출력 파싱 → 정답 대조 → 점수 산정까지 이어지는 **자동 실행·채점 구조(하네스)**를 고정한다.
...
| 단계 | 담당 | 내용 |
|---|---|---|
| 1 | **고정** | **고정 시스템 지시 구성** — 역할 정의·허용 라벨·출력 형식·금지사항·파싱 가능한 형식 답변 지시. 모든 참가자에 동일 적용 |
| 2 | **참가자** | **참가자 행동 지침 삽입** — 컬럼 해석·변수 우선순위·등급/구간 판단 기준·경계 사례·불확실 처리를 지시 |
| 3 | **고정** | **비공개 데이터 입력** — 레코드를 **raw JSON**으로 주입 (파생값·정답 힌트 없이 — 파생은 지침이 결정) |
| 4 | **고정** | **LLM 실행** — 위 3개 입력으로 예측 생성 · 모델/temperature/출력형식 고정 권장(참가자 지침이 핵심 변수) · 초기 설계는 행당 1회, 운영 전 배치 호출 검토 |
| 5 | **고정** | **출력 파싱·검증** — 마지막 `risk_grade, cycle_range` 줄 계약 파싱; 형식 위반·허용값 밖 행 0점/감점 |
| 6 | **고정** | **정답 대조·채점** — Macro F1 · 신뢰성·데이터활용 · 통합 산식 · 원출력·파싱·채점 결과 저장 |

```mermaid
flowchart LR
    SYS["① 고정 시스템 지시<br/>(고정)"] --> RUN
    INS["② 참가자 행동 지침<br/>(참가자)"] --> RUN
    DATA["③ 비공개 데이터 입력<br/>raw JSON (고정)"] --> RUN
    RUN["④ LLM 실행<br/>모델·temperature·seed 고정 권장"] --> S5["⑤ 출력 파싱·검증<br/>(고정)"]
    S5 --> S6["⑥ 정답 대조·채점<br/>(고정)"]
    S6 --> OUT["점수"]
```
...
- **Shape B (2차 PBL 대회 이관):** 참가자가 단계별 지침을 작성하는 **다단계** 파이프라인(예: 전처리 지침 → 분류 지침, 하네스가 체이닝). 풍부하나 비용과 실패모드가 증가한다. 다만 초급 친화성을 우선하면 Shape B가 더 적합할 수 있어 주최측 결정이 필요하다.

> **현재 본 문서는 Shape A를 기준으로 작성한다.** 단, 주최측 확인 결과 Shape B가 채택될 경우 **제출 스키마·하네스 호출 구조·채점 파이프라인·UI가 모두 변경**된다(열린 결정 ④ · §7.0·§7.1).
...
```json
{
  "instruction": "너는 K-511 계열 군용 트럭의 부품 계통별 정비 분석가다. 아래 부품 계통별 정비 이력의 고장위험등급(risk_grade)과 다음 정비주기 구간(cycle_range)을 분류하라.\n입력:\n{{input}}\n출력은 반드시 `<risk_grade>, <cycle_range>` 한 줄로만. risk_grade∈{HIGH,MEDIUM,LOW}, cycle_range∈{0-30,31-90,91-180,181+}.",
  "memo": "선택: 접근 방식 요약"
}
```
...
## 4. 자동채점 설계안 — 배점은 주최측 확정 필요

> ⚠️ **재논의 중 (7/5 자문단 미팅) · 배점 미확정 · 주최측 결정.** 채점은 **리더보드 점수 / 토탈 점수 분리**로 재구성됐다:
> - **리더보드 점수**(실시간 경쟁, 최고점 자동 선택) = 결과값 유효성(예측 정확성·Macro F1) + **프롬프트 효율성**(글자·토큰).
> - **토탈 점수**(최종·수상) = 리더보드 점수 + 온라인 수강률 + 제출형식 준수 + 보안 적합성.
>
> 정확 배점은 **대회 의도·문제 수(1 vs 4)** 확정 후(§7). 아래 §4.1~4.5(IITP 공식 5-dim)는 **참고 기준**이며, 프롬프트 효율성 재도입·분리 반영은 주최측 확정 후 개정.

### 4.1 참고 배점안: IITP 공식 5개 항목 기준

> **아래 배점표는 개발사가 채점 구조를 이해하기 위한 참고안이며, 최종 구현 배점은 주최측 확인 후 확정한다.** 특히 리더보드 점수와 토탈 점수를 분리할 경우, 아래 100점 구조는 그대로 적용되지 않을 수 있다.
...
| 문제 해결 | 문제해결 완성도 | **10** | 커버리지 — 비공개 데이터 전부(Public 300 / Private 700) 유효 예측 비율 (§4.4) |
...
- `risk_grade`(명목형 3-class): **Macro F1**(클래스별 F1 평균).
- `cycle_range`(순서형 4-class): **Macro F1(기본)** vs **QWK / MAE·RMSE**(공식 구현기준 병기) 협의(§7).
- `평균 F1 = (F1_risk + F1_cycle)/2` (0~1) → `40 × 평균 F1`.
- 하네스가 모델 출력을 라벨로 파싱; 파싱 실패·허용 밖은 오분류.
- **Public/Private:** 비공개 데이터 = Public 300 / Private 700. 대회 중 Public, 최종 Private 재채점 → 역추적·과적합 차단.
...
| 항목 | 만점 | 감점 |
|---|---|---|
| 문제해결 완성도 (10·커버리지) | 비공개 데이터(Public 300 / Private 700) 전부 유효 예측(빠짐·빈값 0) | 무응답·파싱 실패 행당 차감 (하한 0) |
...
- **제출 제약:** 지침 3000자 하드캡 초과 → 반려(payload·비용). 위 IITP 참고안 기준에서는 **간결성을 채점하지 않는다**(공식 기준에 없음). ⚠️ 단, §4 배너의 리더보드/토탈 분리안은 **프롬프트 효율성(글자·토큰)을 리더보드 점수에 재도입**하는 방향이라 이 항목과 상충 — 효율성 반영 여부는 주최측 확정 사항(열린 결정 ③ · §7.0).
...
- **점수 소스:** 대회 중 Public(300), 마감 후 Private(700) 최종.
...
flowchart TD
    ...
    V -->|실패| REJ["반려"]
    V -->|통과| SEC["보안 스캔 (+ 플래그)"]
    SEC --> RUN["하네스 재실행<br/>직렬화 → 지정 모델 API(모델·temperature·seed 고정 권장) × 비공개 데이터 (Public 300 / Private 700) → 답변"]
...
1. **채점 모델** — **미확정.** 경량 모델을 우선 검토하되 후보별 실측이 필수다. 비용·속도·응답 안정성에 더해 **변별력**을 함께 확인 후 확정. ⚠️ **모델별 점수 천장 실측**: 동일 규칙 프롬프트가 `gpt-5.4-nano` ≈35 / `gpt-5.4-mini` ≈54 / `gpt-5.4` ≈80 — 소형 고정 시 완벽한 규칙도 상한이 낮아 변별력이 떨어진다. ⚠️ **`gpt-4o-mini`는 더 나쁘다(실측)**: 결합 F1 0.1766으로 다수클래스 상수 예측(0.1711)과 구별되지 않고, 조잡한 422자 프롬프트(0.3102)가 정교한 650자를 **역전**해 점수가 프롬프트 품질과 **역상관**한다. 모델명의 `mini`/`nano`로 성능을 유추하지 말 것(→ `reference_prompts.md`). 상세·비용 → `dev_workplan_scoring_leaderboard.md`
2. **재현성** — 모델 버전·temperature·seed·출력 형식 고정. 완전한 결정론 보장보다는 원출력·파싱 결과·채점 결과 저장으로 검증 기준 확보
3. **API 예산·레이트리밋** — 공식 제출만 단순 계산 시 `1만×3×1,000 = 3,000만 호출/일` 가능. 행당 1회 호출은 비용 리스크가 크므로 5~10행 미니배치 또는 일괄 호출 비교 검토 필요
...
9. **비공개 데이터 문항 수** — 비공개 채점셋은 Public 300 / Private 700(합계 1,000행)로 설계. 리더보드 안정성·변별력을 확보하되, 비용과 배치 채점 구조를 함께 고려
...
```text
참가자 10,000명 × 1일 공식 제출 3회 × 비공개 데이터 1,000행(Public 300 + Private 700)
= 최대 30,000,000회 호출/일
```
...
3. 일괄 호출: 비용은 낮으나 출력 파싱 실패 시 리스크가 큼
