...
- **모델:** non-reasoning 소형(gpt-4o-mini/4.1-nano) + 정답-only 출력 강제(출력 토큰 캡 정합). ⚠️ **모델이 점수 천장을 결정(실측):** 규칙을 다 담은 동일 프롬프트가 `gpt-5.4-nano` ≈35 / `gpt-5.4-mini` ≈54 / `gpt-5.4` ≈80. ⚠️ **`gpt-4o-mini`는 0.1766(23.73)로 다수클래스 상수 예측(0.1711)과 구별 불가**이며, 조잡한 422자 프롬프트(0.3102)가 정교한 650자(0.1766)를 **역전**한다 — 이 모델로 고정하면 점수가 프롬프트 품질과 역상관하므로 모델은 **미확정 유지**. `gpt-4.1-nano`는 0.3131(리더보드 36.01)로 순서가 보존돼 현재까지 최선의 후보. ⚠️ **출력 절단이 품질 사다리를 깨뜨린다(실측)**: 추론 유도 822자 지침(P2 0.1738)이 211자(P1 0.2337)보다 낮고, INVALID 151행이 전부 69~91자에서 절단. ✅ **처방은 `max_tokens` 상향이 아니라 고정 시스템 지시**(200 상향은 실패 0.1813·남은 INVALID 119/119 재절단 / 고정 지시는 캡 50 그대로 INVALID 0.0267·F1 0.2741·P0<P1<P2 단조 회복) → 6단계 하네스 1단계를 실제 구현할 것. **캐시 키에 `max_tokens`·`temperature`·`seed` 미포함** — 파라미터 실험 시 별도 DB+`force` 필수. 실패 셀은 오답 계상되며 5% 임계 아래면 조용히 완주(공정성 결함) — 소형·`max_tokens=50`·정답-only에선 완벽한 규칙도 실행이 안 돼 상한이 낮다(글자수 무관). 참고 프롬프트(≈50·≈80)·실측 → `request/reference_prompts.md`. `llm.py`는 최신 모델용 `max_completion_tokens`↔`max_tokens` 자동 폴백 + 4xx 본문 로깅.
